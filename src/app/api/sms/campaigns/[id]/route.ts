...
import { updateSmsCampaignSchema } from "@/lib/validators/sms";
...
export async function PUT(request: Request, ctx: Ctx) {
  ...
  const scheduledAt = data.scheduledAt !== undefined
    ...
    : undefined;
  const updated = await updateSmsCampaign(id, {
    ...data,
    scheduledAt,
  });
  if (!updated) return jsonErr("NOT_FOUND", "Campanha não encontrada ou não pode ser editada.", 404);
