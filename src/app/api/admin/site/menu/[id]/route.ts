...
export async function PATCH(request: Request, context: Context) {
  ...
  const payload = {
    ...
    parentId: parsed.data.parentId !== undefined ? parsed.data.parentId : existing.parentId,
    isExternal: parsed.data.isExternal ?? existing.isExternal,
  ...
  const item = await prisma.siteMenuItem.update({
    where: { id },
    data: {
      label: payload.label,
      href: payload.href,
