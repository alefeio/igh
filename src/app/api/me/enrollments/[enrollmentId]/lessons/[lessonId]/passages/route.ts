...
/** Lista trechos destacados (marca-texto) da aula para a matrícula do aluno. Apenas STUDENT. */
export async function GET(
  ...
  const passages = await prisma.enrollmentLessonPassage.findMany({
...
/** Cria um trecho destacado. Body: { text: string, startOffset: number }. Apenas STUDENT. */
export async function POST(
  ...
  const startOffset =
    typeof body.startOffset === "number" && body.startOffset >= 0
      ? Math.floor(body.startOffset)
      : 0;
