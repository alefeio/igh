...
export default function AdminForumCourseLessonsPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      setLoading(true);
      try {
        const res = await fetch(`/api/admin/course-forum/${courseId}/lessons`);
  ...
  return (
    <div className="min-w-0">
      <DashboardHero
        eyebrow="Fórum do curso (admin)"
        title={courseName || "Carregando…"}
        description="Escolha a aula para ver os tópicos de todos os alunos matriculados em qualquer turma deste curso."
      />
        ...
        {loading ? (
          ...
          <ul className="space-y-2">
            {lessons.map((l) => (
              <li key={l.lessonId}>
                <Link
                  href={`/admin/forum/${courseId}/aula/${l.lessonId}`}
