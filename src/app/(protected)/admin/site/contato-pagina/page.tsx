...
export default function ContatoPaginaPage() {
  ...
  async function load() {
    setLoading(true);
    try {
      const res = await fetch("/api/admin/site/contato-page");
      const json = (await res.json()) as ApiResponse<{ item: ContatoPageItem | null }>;
      if (!res.ok || !json.ok) {
  ...
  async function save(e: React.FormEvent) {
    ...
    try {
      ...
      const json = (await res.json()) as ApiResponse<{ item?: ContatoPageItem; pending?: boolean; message?: string }>;
      if (!res.ok || !json.ok) {
        toast.push("error", !json.ok ? (json as ApiErr).error.message : "Falha ao salvar.");
        return;
      }
      toast.push("success", siteMutationMessage(json.data, "Conteúdo da página Contato atualizado."));
    } finally {
      setSaving(false);
    }
  }
  ...
  if (loading) {
    return (
      <div className="flex flex-col gap-4">
        <div className="text-lg font-semibold">Contato (página)</div>
    ...
    <div className="flex flex-col gap-4">
      <div>
        ...
        <div className="text-sm text-[var(--text-secondary)]">
          Título, subtítulo e foto de fundo do cabeçalho da página /contato do site.
        </div>
      </div>
      <form className="flex flex-col gap-4" onSubmit={save}>
        ...
        <div>
          ...
          <Input
            ...
            placeholder="Ex: Envie sua mensagem ou inscreva-se nas formações."
          />
        </div>
        <div>
