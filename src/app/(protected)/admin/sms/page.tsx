...
export default function SmsCampaignsPage() {
  ...
  async function load() {
    ...
    try {
      ...
      const res = await fetch(`/api/sms/campaigns?${params}`);
      const json = (await res.json()) as ApiResponse<{ items: Campaign[]; total: number; page: number; pageSize: number }>;
      if (!res.ok || !json.ok) {
  ...
  return (
    <div className="mx-auto max-w-6xl px-4 py-6">
      <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
        <h1 className="text-2xl font-semibold text-[var(--text-primary)]">Campanhas SMS</h1>
        <div className="flex flex-wrap items-center gap-2">
          <Link href="/admin/sms/templates">
            <Button variant="secondary">Templates</Button>
          </Link>
          <Link href="/admin/sms/nova">
            ...
            <Table>
              ...
              <tbody>
                {items.map((c) => (
                  <tr key={c.id}>
                    <Td>
                      <Link href={`/admin/sms/${c.id}`} className="font-medium text-[var(--igh-primary)] hover:underline">
                        {c.name}
                      </Link>
                    </Td>
                    <Td>
