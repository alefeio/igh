...
export function HomeAudiencePathsStrip({
  showPreInscricao = false,
}: {
  /** Exibir atalho de pré-inscrição só quando não há ciclo visível para matrícula. */
  showPreInscricao?: boolean;
}) {
  return (
      ...
      <Container>
        ...
        >
          {showPreInscricao ? (
            <a
              href="/pre-inscricao"
              className="touch-manipulation rounded-lg bg-[var(--igh-accent)] px-4 py-3 text-center text-sm font-semibold text-white shadow-sm transition hover:bg-[var(--igh-accent-hover)] sm:min-w-[220px] sm:py-3.5"
            >
              Pré-inscrição próximo ciclo
              <span className="mt-0.5 block text-xs font-normal text-white/90">
                Deixe seu interesse (sem conta)
              </span>
            </a>
          ) : (
            <a
          ...
          {!showPreInscricao ? (
            ...
            >
              Ver catálogo
              <span className="mt-0.5 block text-xs font-normal text-[var(--igh-muted)]">
                Explorar cursos
              </span>
            </a>
          ) : (
